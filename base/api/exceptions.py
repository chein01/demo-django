...
def my_error_handler(exc, *args):
    ...
    if isinstance(exc, exceptions.APIException):
        ...
        try:
            ...
            message = exc.detail
        return ErrorResponse(message=message, status=exc.status_code, code=code)
    if (
        isinstance(exc, NotFoundException)
        or isinstance(exc, PermissionDeniedException)
        or isinstance(exc, ValidationException)
        or isinstance(exc, BadRequestException)
    ):
        return ErrorResponse(
            message=CustomAPIException.get_message(exc),
            status=CustomAPIException.get_status_code(exc),
            data=CustomAPIException.get_data(exc),
        )
...
class CustomAPIException(Exception):
    def __init__(
        self, message="Exception", status_code=status.HTTP_400_BAD_REQUEST, data=None
    ):
        if message:
...
class PermissionDeniedException(CustomAPIException):
    ...
    def __init__(self, message=message, status_code=status_code):
        super(PermissionDeniedException, self).__init__(
            message=message, status_code=status_code
        )
