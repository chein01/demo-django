...
class RetrieveModelMixin(object):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        return SuccessResponse(
            data=data, code=success_response[0], message=success_response[1]
        )
...
class ListModelMixin(object):
    ...
    def get_success_response(self, serializer):
        success_response = self.success_response
        return SuccessResponse(
            data=serializer.data, code=success_response[0], message=success_response[1]
        )
...
class UpdateModelMixin:
    ...
    def update(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(
            instance, data=credentials, context={"request": request}, partial=partial
        )
        if not serializer.is_valid():
            # write log error response
            return ValidationErrorResponse(data=serializer.errors)
...
class DestroyModelMixin:
    ...
    def get_success_response(self, instance=None):
        ...
        return SuccessResponse(
            status=self.status_code,
            code=success_response[0],
            message=success_response[1],
        )
...
class ListModelNotPageMixin(object):
    success_response = MSG_SUCCESS
...
class ListModelNotPageFilterByPkMixin(object):
    success_response = MSG_SUCCESS
    ...
    def get_pk(self):
        return self.kwargs["pk"] if "pk" in self.kwargs else None
